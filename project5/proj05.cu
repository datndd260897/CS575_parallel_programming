...
const float GMIN =	20.0f;	// ground distance in meters
const float GMAX =	30.0f;	// ground distance in meters
const float HMIN =	10.0f;	// cliff height in meters
const float HMAX =	20.0f;	// cliff height in meters
const float DMIN  =	10.0f;	// distance to castle in meters
const float DMAX  =	20.0f;	// distance to castle in meters
const float VMIN  =	10.0f;	// intial cannonball velocity in meters / sec
const float VMAX  =	30.0f;	// intial cannonball velocity in meters / sec
const float THMIN = 	70.0f;	// cannonball launch angle in degrees
const float THMAX =	80.0f;	// cannonball launch angle in degrees

// constants:
const float GRAVITY =	-9.8f;	// acceleration due to gravity in meters / sec^2
const float TOL     = 	 5.0f;	// tolerance in cannonball hitting the castle in meters
...
Radians( float d )
{
	return (3.14159265358979323846f/180.f) * d;
}
...
MonteCarlo( float *dvs, float *dths, float *dgs, float *dhs, float *dds, int *dhits )
{
	...
	float t = -vy / ( 0.5f*GRAVITY );	// time to apex
	float x = vx * t;
	if( x >= g )
	{
		...
		if( y >= 0.0f && y <= h )
		{
			// the ball hits the cliff face, no castle hit
			dhits[gid] = 0;
			return;
		}
		...
		// see if the ball hits the castle (within tolerance)
	...
	{
		// ball doesn't reach cliff, no hit
		dhits[gid] = 0;
	}
}
